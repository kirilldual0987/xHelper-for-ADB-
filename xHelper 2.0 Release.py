...
APP_VERSION = "2.0"
APP_NAME = "xHelper"
CONFIG_PATH = Path.home() / ".xhelper_config.json"
...
class XHelperMainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"{APP_NAME} {APP_VERSION} Release")
    ...
    # ------------------------------------------------------------------
    #   Настройки и ADB helpers
    # ------------------------------------------------------------------
    def load_settings(self):
        defaults = {"adb_path": "adb", "fastboot_path": "fastboot", "theme_dark": False}
        try:
            ...
            print(f"Не удалось загрузить настройки xHelper: {e}")
        return defaults
    ...
    def save_settings(self):
        try:
            ...
            self.log_message(f"Не удалось сохранить настройки: {e}")

    def adb_executable(self):
        return self.settings.get("adb_path") or "adb"

    def fastboot_executable(self):
        return self.settings.get("fastboot_path") or "fastboot"

    def get_selected_device_id(self):
        current = self.device_list.currentItem() if hasattr(self, "device_list") else None
        return current.text() if current else None

    def build_adb_args(self, command: str, device_id: str | None = None):
    ...
    def run_adb_capture(self, command: str, *, device_specific: bool = True, timeout: int = 30):
        device_id = self.get_selected_device_id() if device_specific else None
        if device_specific and not device_id:
            self.log_message("Не выбрано устройство")
            return subprocess.CompletedProcess([], 1, "", "Не выбрано устройство")
        return subprocess.run(
            self.build_adb_args(command, device_id),
            capture_output=True,
            text=True,
            timeout=timeout,
        )
    ...
    def create_menu(self):
        ...
        self.toggle_dark_action.setChecked(bool(self.settings.get("theme_dark")))
        self.toggle_dark_action.triggered.connect(self.toggle_dark_theme)
        view_menu.addAction(self.toggle_dark_action)
        if self.toggle_dark_action.isChecked():
            QTimer.singleShot(0, self.apply_dark_palette)

    def toggle_dark_theme(self, checked: bool):
        self.settings["theme_dark"] = bool(checked)
        self.save_settings()
        if checked:
    ...
    def check_adb(self):
        ...
        try:
            result = subprocess.run([self.adb_executable(), '--version'],
                                    capture_output=True,
                                    text=True)
    ...
    def run_adb_command(self, command: str, device_specific: bool = True):
        ...
        for dev in devices:
            full_cmd = self.build_adb_args(command, dev)
            try:
            ...
            for i, apk_path in enumerate(self.apk_files):
                ...
                try:
                    result = subprocess.run(
                        self.build_adb_args(f'install -r {shlex.quote(apk_path)}', self.get_selected_device_id()),
                        capture_output=True,
                        text=True,
    ...
    def get_user_packages(self):
        ...
        try:
            result = subprocess.run(
                self.build_adb_args("shell pm list packages -3", self.get_selected_device_id()),
                capture_output=True,
                text=True,
                check=True,
                encoding='utf-8',
    ...
    def test_application(self, package_name: str) -> dict:
        ...
        try:
            subprocess.run(self.build_adb_args("logcat -c", self.get_selected_device_id()), capture_output=True)

            subprocess.run(
                self.build_adb_args(f"shell monkey -p {shlex.quote(package_name)} -c android.intent.category.LAUNCHER 1", self.get_selected_device_id()),
                capture_output=True,
                timeout=5
            )
            time.sleep(3)
            ...
            log = subprocess.run(
                self.build_adb_args("logcat -d -v brief *:E", self.get_selected_device_id()),
                capture_output=True,
                text=True,
                timeout=10
            )
            ...
            subprocess.run(self.build_adb_args(f"shell am force-stop {shlex.quote(package_name)}", self.get_selected_device_id()),
                           capture_output=True)
    ...
    def uninstall_package(self, package_name: str) -> bool:
        try:
            result = subprocess.run(
                self.build_adb_args(f"uninstall {shlex.quote(package_name)}", self.get_selected_device_id()),
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def take_screenshot(self):
        ...
        try:
            with open(file_path, "wb") as f:
                subprocess.run(
                    self.build_adb_args("exec-out screencap -p", self.get_selected_device_id()),
                    stdout=f,
                    check=True
                )
    ...
    def update_monitor(self):
        ...
        # Battery
        bat = subprocess.run(
            self.build_adb_args("shell dumpsys battery", self.get_selected_device_id()),
        ...
        # Memory
        mem = subprocess.run(
            self.build_adb_args("shell cat /proc/meminfo", self.get_selected_device_id()),
            capture_output=True, text=True
        ).stdout
        total = free = None
        ...
        ipinfo = subprocess.run(
            self.build_adb_args("shell ip -f inet addr show wlan0", self.get_selected_device_id()),
            capture_output=True, text=True
        ).stdout
        ip = "?"
    ...
    def start_screen_record(self):
        ...
        self.screenrecord_process = subprocess.Popen(
            self.build_adb_args("shell screenrecord /sdcard/xHelper_record.mp4", self.get_selected_device_id())
        )
