...
class XHelperMainWindow(QMainWindow):
    ...
    def get_selected_device_id(self):
        current = (
            self.device_list.currentItem() if hasattr(self, "device_list") else None
        )
    ...
    def run_adb_capture(
        self, command: str, *, device_specific: bool = True, timeout: int = 30
    ):
    ...
    def check_adb(self):
        ...
        try:
            result = subprocess.run(
                [self.adb_executable(), "--version"], capture_output=True, text=True
            )
    ...
    def get_devices(self):
        ...
        result = subprocess.run(
            [self.adb_executable(), "devices"], capture_output=True, text=True
        )
    ...
    def run_adb_package_command(self, base_cmd: str):
        ...
        pkg, ok = QInputDialog.getText(
            self, "Имя пакета", "Введите полное имя пакета (например, com.example.app):"
        )
        if ok and pkg:
    ...
    def install_apks_thread(self):
        ...
        with open(log_file, "w", encoding="utf-8") as log_f:
            ...
            for i, apk_path in enumerate(self.apk_files):
                ...
                try:
                    result = subprocess.run(
                        self.build_adb_args(
                            f"install -r {shlex.quote(apk_path)}",
        ...
        self.log_signal.emit(
            f"Установка завершена! Успешно: {success}, Ошибки: {failed}"
        )

        if failed == 0:
            QMessageBox.information(
                self, "Готово", "Все APK‑файлы установлены успешно!"
            )
        else:
            QMessageBox.warning(
                self,
    ...
    def create_app_tester_tab(self):
        ...
        self.app_tree.header().setSectionResizeMode(
    ...
    def get_user_packages(self):
        ...
        try:
            result = subprocess.run(
                self.build_adb_args(
                    "shell pm list packages -3", self.get_selected_device_id()
                ),
                capture_output=True,
                text=True,
                check=True,
            ...
            if result.stdout:
                ...
                self.log_message(
                    f"Найдено {len(self.packages)} пользовательских приложений"
                )
            ...
            QMessageBox.critical(
                self, "Ошибка", f"Не удалось получить список приложений:\n{e}"
            )
    ...
    def test_applications_thread(self):
        try:
            ...
            for i, pkg in enumerate(self.packages):
                ...
                if result["crashed"]:
                    self.update_app_test_status(
                        i, f"Ошибок: {result['error_count']}", "red"
                    )
                ...
                for sec in range(delay, 0, -1):
                    if not self.testing:
                        break
                    self.log_signal.emit(
                        f"Ожидание {sec} сек. перед следующим тестом..."
                    )
                    time.sleep(1)

            if self.crashed_apps:
                self.log_signal.emit(
                    f"Тестирование завершено. Проблемных приложений: {len(self.crashed_apps)}"
                )
            else:
                self.log_signal.emit(
                    "Тестирование завершено. Проблемных приложений не обнаружено"
                )
        except Exception as e:
    ...
    def test_application(self, package_name: str) -> dict:
        ...
        try:
            subprocess.run(
                self.build_adb_args("logcat -c", self.get_selected_device_id()),
                capture_output=True,
            )

            subprocess.run(
                self.build_adb_args(
                    f"shell monkey -p {shlex.quote(package_name)} -c android.intent.category.LAUNCHER 1",
                    self.get_selected_device_id(),
                ),
                capture_output=True,
                timeout=5,
            ...
            log = subprocess.run(
                self.build_adb_args(
                    "logcat -d -v brief *:E", self.get_selected_device_id()
                ),
                capture_output=True,
                text=True,
                timeout=10,
            ...
            subprocess.run(
                self.build_adb_args(
                    f"shell am force-stop {shlex.quote(package_name)}",
                    self.get_selected_device_id(),
                ),
                capture_output=True,
            )
    ...
    def delete_selected_apps(self):
        ...
        QMessageBox.information(
            self, "Готово", f"Удалено {success} из {len(selected)} приложений"
        )
    ...
    def delete_all_problematic_apps(self):
        ...
        QMessageBox.information(
            self, "Готово", f"Удалено {success} проблемных приложений"
        )
        self.delete_selected_btn.setEnabled(False)
        self.delete_all_btn.setEnabled(False)
    ...
    def uninstall_package(self, package_name: str) -> bool:
        try:
            result = subprocess.run(
                self.build_adb_args(
                    f"uninstall {shlex.quote(package_name)}",
                    self.get_selected_device_id(),
                ),
                capture_output=True,
                text=True,
            ...
            if result.stdout and "Success" in result.stdout:
                ...
                self.log_message(
                    f"Не удалось удалить {package_name}: {result.stdout or result.stderr}"
                )
                return False
        except subprocess.CalledProcessError as e:
    ...
    def take_screenshot(self):
        ...
        try:
            with open(file_path, "wb") as f:
                subprocess.run(
                    self.build_adb_args(
                        "exec-out screencap -p", self.get_selected_device_id()
                    ),
                    stdout=f,
                    check=True,
                )
    ...
    def update_monitor(self):
        ...
        mem = subprocess.run(
            self.build_adb_args(
                "shell cat /proc/meminfo", self.get_selected_device_id()
            ),
            capture_output=True,
            text=True,
        ).stdout
        ...
        ipinfo = subprocess.run(
            self.build_adb_args(
                "shell ip -f inet addr show wlan0", self.get_selected_device_id()
            ),
            capture_output=True,
            text=True,
        ).stdout
    ...
    def create_wifi_tab(self):
        ...
        self.wifi_ip_input.setPlaceholderText(
            "IP‑адрес устройства (пример: 192.168.1.42)"
        )
    ...
    def create_backup(self):
        ...
        self.run_adb_command(
            f"backup -apk -shared -all -f {file_path}", device_specific=False
        )
    ...
    def start_screen_record(self):
        ...
        self.screenrecord_process = subprocess.Popen(
            self.build_adb_args(
                "shell screenrecord /sdcard/xHelper_record.mp4",
                self.get_selected_device_id(),
            )
        )
    ...
    def save_screen_record(self):
        ...
        self.run_adb_command(
            f"pull /sdcard/xHelper_record.mp4 {save_path}", device_specific=False
        )
        self.run_adb_command(
            "shell rm /sdcard/xHelper_record.mp4", device_specific=False
        )
    ...
    def fastboot_devices(self):
        try:
            result = subprocess.run(
                ["fastboot", "devices"], capture_output=True, text=True, timeout=15
            )
    ...
    def select_flash_file(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Выберите файл для flash", "", "All Files (*)"
        )
        if path:
    ...
    def generate_test_report(self):
        ...
        for pkg in self.packages:
            if pkg in self.crashed_apps:
                ...
                entry = {"package": pkg, "status": "ok", "details": "No errors"}
        ...
        QMessageBox.information(
            self, "Отчёт", "Отчёт о тестировании сохранён в текущей папке."
        )
