...
def _run_adb(main_window, cmd):
    ...
    try:
        out = subprocess.check_output(main_window.build_adb_args(cmd, main_window.get_selected_device_id()) if hasattr(main_window, "build_adb_args") else [adb] + shlex.split(cmd), text=True, timeout=10)
        return out
